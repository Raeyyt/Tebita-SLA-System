...
export const DivisionDashboardPage = () => {
    const { token } = useAuth();
    ...
    useEffect(() => {
        const loadData = async () => {
            if (!token) return;
            try {
                const dashboardData = await api.getDivisionDashboard(token);
    ...
    return (
        <div className="container">
            <div className="page-header">
                <h1 className="page-title">{data.division.name}</h1>
                <p className="page-subtitle">Division Manager Dashboard</p>
            </div>
